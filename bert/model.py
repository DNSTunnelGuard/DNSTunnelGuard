...
PRETRAINED_MODEL_NAME = "model_weights/two_pass"
# Tokenizer is safe to load at import time — it's lightweight and has no
# local path dependency unlike the model weights
...
def train_and_save(train_dataset: Dataset, val_dataset: Dataset, output_name: str, model_name: str = "Default", arg_type: str = "Default") -> None:
    ...
    if model_name == "Default":
        training_model = AutoModelForSequenceClassification.from_pretrained(DEFAULT_MODEL_NAME, num_labels=2)
    else:
        training_model = AutoModelForSequenceClassification.from_pretrained(PRETRAINED_MODEL_NAME, num_labels=2)
